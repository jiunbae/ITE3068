...
for server in $ADDRS;
do
    sshpass -p "REMOVED-CREDENTIAL" scp -o StrictHostKeyChecking=no /root/.ssh/authorized_keys $server:/root/.ssh/
...
sed -i 's/REMOVED-CREDENTIAL -E/REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -E/g' arcus.sh
sed -i 's/REMOVED-CREDENTIAL -E/REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -E/g' etc/arcus.sh
sed -i 's/REMOVED-CREDENTIAL -E/REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -E/g' fabfile.py
