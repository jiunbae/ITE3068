...
def mc_start_server(config, zkhosts):
  exe = '%s/bin/memcached -u memcached -u memcached -E %s/lib/default_engine.so -X %s/lib/syslog_logger.so -X %s/lib/ascii_scrub.so -d -v -r -R5 -U 0 -D: -b 8192 -m%s -p %s -c %s -t %s -z %s'%(
