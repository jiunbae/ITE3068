...
def inject_zkclient_and_config():
  ...
  The wrapped function should have the following arguments:
    ...
    - context    : (injected) context
        ...
        mclist     - REMOVED-CREDENTIAL list -> (ip, port, config)
        ipset      - set of IPs for all servers (zookeeper+REMOVED-CREDENTIAL)
    ...
    def wrapped(*args, **kargs):
      ...
      if configfile is not None:
        ...
        for server in config.get('servers', []):
          ...
          ctx_ipset.add(ip)
          fab_roledefs['REMOVED-CREDENTIAL'].append(ip)
...
@task
@roles('zookeeper', 'REMOVED-CREDENTIAL')
@parallel
def deploy():
...
def mc_start_server(config, zkhosts):
  exe = '%s/bin/REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -E %s/lib/default_engine.so -X %s/lib/syslog_logger.so -X %s/lib/ascii_scrub.so -d -v -r -R5 -U 0 -D: -b 8192 -m%s -p %s -c %s -t %s -z %s'%(
          ARCUS_PATH, ARCUS_PATH, ARCUS_PATH, ARCUS_PATH,
          config['memlimit'], config['port'], config['connections'], config['threads'], zkhosts
        )
  try:
    # REMOVED-CREDENTIAL process would be blocked (I don't know why..) so just timeout it
...
def mc_stop_server(config):
  exe = "ps -ef | grep -e REMOVED-CREDENTIAL | grep -e '-p %s' | grep -v 'ssh' | awk '{print $2}'"%config['port']
...
def ping(service_code):
  ...
  REMOVED-CREDENTIAL_servers = set([ each['ip'] for each in cluster['servers'] ])
  for each in REMOVED-CREDENTIAL_servers:
