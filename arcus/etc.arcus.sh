...
ARCUS_SERVER=$PKG_DIR/bin/REMOVED-CREDENTIAL
...
case $MODULE in
  ...
  "server")
    ...
    case $MODE in
      ...
      # start
      $PKG_DIR/bin/REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -u REMOVED-CREDENTIAL -E $PKG_DIR/lib/default_engine.so -X $PKG_DIR/lib/syslog_logger.so $PKG_DIR/lib/ascii_scrub.so -d -v -r -R5 -U 0 -b 8192 -m$memlimit -p $port -c $connections -t $threads -z $ZK_HOSTPORT
      ;;
      ...
      procnum=`ps -ef | grep -e REMOVED-CREDENTIAL | grep -e "-p $port" | awk '{print $2}'`
      if [ -z "$procnum" ]; then
